...
import { UserRecordSchema } from "prisma/generated";
	...
	import { ZodEnum, ZodNullable, ZodNumber, ZodOptional, ZodSchema, ZodString, type ZodTypeAny } from "zod";
...
function unwrap(schema: ZodTypeAny, ignore: any[] = [ZodNullable, ZodOptional]): ZodTypeAny {
...
async function search() {
	const search = Array.from(searchTerm.trim().matchAll(/@(.*?):(.*?)(?: |$)/g))
	tagDropdownOpen = false;

	const filters = search.reduce((acc, [_, key, value]) => {
		if (zodValidator.shape.hasOwnProperty(key)) {
			if (isNumberSchema(zodValidator.shape[key])) {
				const { success, data } = zodValidator.shape[key].safeParse(parseFloat(value.trim()));
...
function keydown(e: CustomEvent<KeyboardEvent>) {
	...
	if (e.detail.key === "Enter") {
		...
		const previousText = editor.state.doc.slice(0, selection.from).toString();
		// Go back until we hit an "@" sign or a space
		let from = selection.from;
		while (from > 0 && previousText.charAt(from - 1) !== "@" && previousText.charAt(from - 1) !== " ") {
			from--;
		}
		...
		const matches = validTags.filter(tag => tag.startsWith(partialName));
...
const stringMatcher = new RegExp(`(?:${validTags.map(tag => `@${tag}:`).join("|")})`, "g");
...
const isZodNumber = (schema: ZodTypeAny) => unwrap(schema) instanceof ZodNumber;
const isZodEnum = (schema: ZodTypeAny) => unwrap(schema) instanceof ZodEnum;
const isZodString = (schema: ZodTypeAny) => unwrap(schema) instanceof ZodString;
const isZodOptional = (schema: ZodTypeAny) => unwrap(schema, [ZodNullable]) instanceof ZodOptional;
const isZodNullable = (schema: ZodTypeAny) => unwrap(schema, [ZodOptional]) instanceof ZodNullable;
</script>
...
<div class="grid grid-cols-2">
	...
	<div class="flex flex-row gap-4">
		<FilterInput bind:value={searchTerm} bind:editor={editor} validTags={validTags} placeholder="Search..." class="w-full" on:keydown={keydown} on:focusin={() => tagDropdownOpen = true} on:focusout={() => tagDropdownOpen = false} on:update={update}></FilterInput>
		<div class="dropdown dropdown-left top-16 right-4" class:dropdown-open={tagDropdownOpen}>
			<div class="dropdown-content menu bg-base-100 rounded-lg z-[1] w-96 p-4 border border-base-300 flex flex-col gap-4">
				{#each validTags as tag}
					{#if searchTerm.includes(`@${tag}:`)}
						{@const match = searchTerm.match(new RegExp(`@${tag}:(.+?)(?:\\s|$)`))}
						{@const zodProperty = zodValidator.shape[tag]}
							<div class="flex flex-row items-center gap-2">
								<span class="text-base-content text-opacity-50">{`${tag}${(isZodOptional(zodProperty) || isZodNullable(zodProperty)) ? "?" : ""}`}</span>
								<span class="text-base-content text-opacity-50">
									{#if match}
										{#if isZodEnum(zodProperty)}
											{@const partialMatch = Object.keys(unwrap(zodProperty).Enum).filter(key => key.startsWith(match[1]))}
											{#if partialMatch.length > 0}
												{#each partialMatch as option}
													<span class="badge badge-sm badge-neutral">{option}</span>
												{/each}
											{:else}
												<span class="badge badge-sm badge-neutral">{match[1]}</span>
											{/if}
										{:else}
											<span class="badge badge-sm badge-neutral">{match[1]}</span>
										{/if}
									{:else if isZodNumber(zodProperty)}
										<span class="badge badge-sm badge-neutral">number</span>
									{:else if isZodString(zodProperty)}
										<span class="badge badge-sm badge-neutral">string</span>
									{:else if isZodEnum(zodProperty)}
										{#each Object.keys(unwrap(zodProperty).Enum) as keys}
											<span class="badge badge-sm badge-neutral">{keys}</span>
										{/each}
									{/if}
								</span>
							</div>
		...
		<div class="dropdown dropdown-left top-16 right-4" class:dropdown-open={dropdownOpen}>
			<div class="dropdown-content menu bg-base-100 rounded-lg z-[1] max-w-[576px] w-max p-4 border border-base-300 ">
				{#if result && result.records.length > 0}
					<SimilarRecords similar={result.records}></SimilarRecords>
				{:else}
					<div class="flex flex-col items-center justify-center gap-4">
						<span class="text-base-content text-opacity-50">No results found</span>
					</div>
				{/if}
			</div>
		</div>
